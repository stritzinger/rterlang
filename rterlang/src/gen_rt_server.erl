...
try_handle_overload(Mod, Msg, State) ->
    try
	{ok, Mod:handle_overload(Msg, State)}
    catch
	throw:R ->
	    {ok, R};
	Class:R ->
...
try_handle_dlmiss(Mod, State) ->
    try
	{ok, Mod:handle_dlmiss(State)}
    catch
	throw:R ->
	    {ok, R};
	Class:R ->
...
system_terminate(Reason, _Parent, Debug, [Name, State, _Deadline, Mod, _Time, _HibernateAfterTimeout]) ->
    terminate(Reason, ?STACKTRACE(), Name, undefined, [], Mod, State, Debug).

system_code_change([Name, State, _Deadline, Mod, Time, HibernateAfterTimeout], _Module, OldVsn, Extra) ->
    case catch Mod:code_change(OldVsn, State, Extra) of
	{ok, NewState} -> {ok, [Name, NewState, Mod, Time, HibernateAfterTimeout]};
	Else -> Else
    end.

system_get_state([_Name, State, _Deadline, _Mod, _Time, _HibernateAfterTimeout]) ->
    {ok, State}.

system_replace_state(StateFun, [Name, State, _Deadline, Mod, Time, HibernateAfterTimeout]) ->
