...
start_link() ->
    gen_rt_server:start_link({local, ?SERVER}, ?MODULE, [],[
							    {scheduler, rt_sched},
							    {deadline, 1000*1000*1000*4} %nanoseconds
]).
...
handle_call(_Request, _From, State) ->
    Reply = ok,
    timer:sleep(3000),
    io:format("~p Handling call~n", [?MODULE]),
    {reply, Reply, State}.

handle_overload(_Request, State) ->
    io:format("~p OVERLOAD~n", [?MODULE]),
    {stop, State}.

handle_dlmiss(_State) ->
    io:format("~p DL MISSED~n", [?MODULE]),
    exit(rt_dl_missed).
%    {stop, State}.
%%--------------------------------------------------------------------
%% @private
...
handle_cast(_Msg, State) ->
    io:format("~p Handling cast~n", [?MODULE]),
    timer:sleep(3000),
    {noreply, State}.
