...
ErlDrvData grisp_ir_start(ErlDrvPort port, char *command)
{
    ...
    /* 	&grisp_ir_handle, data); */
        
    /* /\* Start task to receive interrupts *\/ */
    /* taskname = rtems_build_name('G','I','P','T'); */
    /* assert(rtems_task_create(taskname, 8, 1024*4, RTEMS_DEFAULT_MODES, RTEMS_DEFAULT_ATTRIBUTES, &(data->tid)) == RTEMS_SUCCESSFUL); */
    /* //return ERL_DRV_ERROR_GENERAL; */
    /* assert(rtems_task_start(data->tid, &grisp_ir_pin_trigger, (rtems_task_argument) Pins)  == RTEMS_SUCCESSFUL); */
    /* //        return ERL_DRV_ERROR_GENERAL; */
...
void grisp_ir_output(ErlDrvData arg, char *buf, ErlDrvSizeT len){
  ...
  uint8_t index, cmd, type, attribute;
  rtems_name taskname;
...
rtems_task grisp_ir_pin_trigger(rtems_task_argument arg){
  ...
  while(rtems_event_receive(RTEMS_EVENT_5 | RTEMS_EVENT_6, RTEMS_WAIT, RTEMS_NO_TIMEOUT, &eset) == RTEMS_SUCCESSFUL){
